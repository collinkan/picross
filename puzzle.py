...
class Puzzle:
    ...
    def select(self, x: int, y: int) -> None:
        if self.get_grid()[x][y] == 0: 
            self.grid[x][y] = 1
            self.grid_copy[x][y] = 1
        elif self.get_grid()[x][y] == 1: 
            self.grid[x][y] = 0
            self.grid_copy[x][y] = 0
    ...
    def solved(self) -> bool:
        for i in range(len(self.get_grid_copy())):
            for j in range(len(self.get_grid_copy()[i])):
                if self.get_grid_copy()[i][j] != self.get_solution()[i][j]:
                    return False
        return True
